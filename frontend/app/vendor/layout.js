...
const NAV_ITEMS = [
  ...
  { href: '/vendor/orders', label: 'Customer Orders', icon: ShoppingBag },
  { href: '/vendor/rental-requests', label: 'Rental Requests', icon: ClipboardList },
  { href: '/vendor/delivery-partners', label: 'Delivery Partners', icon: Truck },
  { href: '/vendor/notifications', label: 'Notifications', icon: Bell },
  { href: '/vendor/messages', label: 'Messages', icon: MessageSquare },
  { href: '/vendor/analytics', label: 'Analytics', icon: BarChart3 },
  { href: '/vendor/profile', label: 'Vendor Profile', icon: Building2 },
    ...
    <div className="flex h-full flex-col">
      ...
      <nav className="mt-8 flex flex-col gap-1.5 overflow-y-auto">
        {NAV_ITEMS.map((item) => {
          ...
          const badge = item.href === '/vendor/notifications' ? unreadCount : 0;
...
export default function VendorLayout({ children }) {
  ...
  const showBackButton = pathname !== '/vendor';

  useEffect(() => {
    if (status === 'unauthenticated') router.replace('/login');
    else if (status === 'authenticated' && user?.role !== 'vendor') router.replace('/');
  }, [status, user, router]);
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        {showBackButton && (
          <div className="border-b border-slate-200/70 px-4 py-2.5 dark:border-white/10 lg:hidden">
            <BackButton fallbackHref="/vendor" />
          </div>
        )}
        ...
        <div className="hidden items-center justify-between gap-3 border-b border-slate-200/70 px-6 py-3 dark:border-white/10 lg:flex">
          <div>{showBackButton && <BackButton fallbackHref="/vendor" />}</div>
