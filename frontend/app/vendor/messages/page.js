...
import { MessageSquare, Send, ImageOff } from 'lucide-react';
import Card from '@/components/ui/Card';
import Skeleton from '@/components/ui/Skeleton';
import { useGetMyVendorProfileQuery, useListMyProductsQuery } from '@/store/vendorApi';
import { buildVendorMessages } from '@/lib/mockVendorData';
...
export default function VendorMessagesPage() {
  ...
  const { data: productsData, isLoading } = useListMyProductsQuery(
    { vendor: vendorId, city: selectedCity?.id, sort: 'newest', limit: 12 },
    { skip: !vendorId }
  );

  const products = productsData?.data?.items || [];
  const threads = useMemo(() => buildVendorMessages(products, 10), [products]);
  const [activeId, setActiveId] = useState(null);
  const [draft, setDraft] = useState('');
  // Vendor replies sent this session, keyed by thread id — merged on top of each thread's
  // seeded conversation history for display, so a reply appears in the chat immediately
  // instead of only surfacing as a toast.
  ...
  const handleSend = () => {
    ...
    const message = { id: `local-${Date.now()}`, sender: 'vendor', text: draft.trim(), timestamp: new Date() };
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Conversations with customers about your listings.</p>
      </div>
        ...
        <Card variant="glass" className="max-h-[36rem] overflow-y-auto p-2 lg:col-span-1">
          {threads.map((t) => {
            ...
            return (
              ...
              >
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-brand-500 to-accent-500 text-xs font-bold text-white">
                  {initials(t.customerName)}
                </span>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <p className="truncate text-sm font-semibold text-slate-900 dark:text-white">{t.customerName}</p>
          ...
          {active && (
            <>
              <div className="flex items-center gap-3 border-b border-slate-200/70 p-4 dark:border-white/10">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">{active.customerName}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Re: {active.product.name}</p>
                </div>
              </div>
                  ...
                  {activeMessages.map((msg, i) => {
                    ...
                    return (
                      <div key={msg.id}>
                        ...
                        >
                          <div className={`flex max-w-[80%] flex-col gap-1 ${isVendor ? 'items-end' : 'items-start'}`}>
                            <div
